 ...
 */
struct gamma {
    uint32_t b_width;///< szerokość planszy.
    uint32_t b_height;///< wysokość planszy.
    uint32_t b_players;///< liczba graczy.
    uint32_t areas_limit;///< limit obszarów.
    uint32_t *busy_areas;///< tablica liczb zajętych przez graczy obszarów.
    bool *golden_move_available;
    /**<
     * Tablica, przechowująca informację, czy dany gracz wykonał złoty ruch.
     * Jeśli nie wykonał, w tablicy pod indeksem [numer_gracza - 1] znajduje
     * się @p true, w przeciwnym przypadku @p false.
     */
    uint32_t **board;///<plansza.
};
///Struktura, przechowująca stan gry.
 ...
 * Sprawdza, czy gracz @p player nie wykonał jeszcze złotego ruchu.
 * Sprawdza również, czy istnieje co najmniej jedno pole, na którym stoi pionek
 * innego gracza, gdzie gracz @p player może postawić swój pionek, nie
 * rozdzielając dotychczasowych obszarów gracza zajmującego przeglądane w danym 
 * momencie pole w taki sposób, by finalna liczba tych obszarów przekraczała 
 * wartość @p g->areas_limit.
 * Sprawdza też, czy po zrobieniu złotego ruchu gracz @p player nie przekroczy 
 * limitu obszarów.
 * @param[in] g       – wskaźnik na strukturę przechowującą stan gry,
 * @param[in] player  – numer gracza, liczba dodatnia niewiększa od wartości
 *                      @p players z funkcji @ref gamma_new.
